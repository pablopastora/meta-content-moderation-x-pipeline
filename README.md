...
Due to ethical and copyright restrictions surrounding social media data, **no raw social media content is shared in this repository**. The scripts provided here allow other researchers to replicate the data collection and analysis procedures in accordance with platform policies.
...
2. **`02_build_urls_meta.R`**  
   Cleans and structures the scraped dataset, generating a table of tweet URLs and associated metadata (media outlet, country, publication date).
...
The `data/` directory is intentionally empty. It is included to document the expected structure but does not contain any datasets.
...
- A dataset of user replies linked to media posts.

- Polarity scores derived from sentiment analysis of translated user replies.

No raw social media texts are distributed in this repository.

## Ethical considerations and data sharing

The underlying data to this research cannot be shared due to ethical and copyright restrictions surrounding social media data.

In accordance with X’s terms of service:

- No tweet texts or replies are reproduced.

- No screenshots or representative posts are included.

The Methods section of the article and the scripts in this repository provide sufficient detail to allow replication of the study. Any queries about the methodology should be directed to the corresponding author.

Where appropriate, Tweet IDs or URLs may be shared upon reasonable request to allow data rehydration via the platform’s API.

## License

This repository is released under a Creative Commons Attribution 4.0 International (CC BY 4.0) license.
...
Pastora Estébanez, P. (2026). Data collection and analysis pipeline for media reactions on X to Meta’s content moderation policy. GitHub repository / Zenodo archive. DOI.
